...
// GenerateOSDOpEventsHTML generates HTML for OSD Op events analysis
func GenerateOSDOpEventsHTML(result types.AnalysisResult) string {
	html := `
    <h2 data-zh="osd_op 事件分析" data-en="osd_op Events Analysis">osd_op 事件分析</h2>
    <div class="layout">
        <div class="left-panel">
            <div class="query-principle">
                <h3 data-zh="查询原理" data-en="Query Principle">查询原理</h3>
                <p data-zh="此分析解析 Ceph 日志中的 OSD op 事件。它识别：" data-en="This analysis parses OSD op events from the Ceph log. It identifies:">此分析解析 Ceph 日志中的 OSD op 事件。它识别：</p>
                <ul>
                    <li data-zh="<strong>开始事件</strong>：包含 "enqueue_op" 和 "osd_op" 的日志行" data-en="<strong>Start events</strong>: Log lines containing "enqueue_op" with "osd_op">开始事件：包含 "enqueue_op" 和 "osd_op" 的日志行</li>
                    <li data-zh="<strong>结束事件</strong>：包含 "sending reply on" 和 "osd_op" 的日志行" data-en="<strong>Finish events</strong>: Log lines containing "sending reply on" with "osd_op">结束事件：包含 "sending reply on" 和 "osd_op" 的日志行</li>
                </ul>
                <p data-zh="对于每个 osd_op 事件，它提取：" data-en="For each osd_op event, it extracts:">对于每个 osd_op 事件，它提取：</p>
                <ul>
            ...
            <div class="summary">
                ...
                <p data-zh="总 osd_op 事件数：" data-en="Total osd_op events: ">总 osd_op 事件数：` + strconv.Itoa(result.TotalEvents) + `</p>`
	...
	html += `
        ...
        <div class="right-panel">
            <div class="filter-form">
                ...
                <input type="datetime-local" id="osdop-start-time">
                <label data-zh="结束时间：" data-en="End Time:">结束时间：</label>
                <input type="datetime-local" id="osdop-end-time">
                <label data-zh="最小持续时间 (ms)：" data-en="Min Duration (ms):">最小持续时间 (ms)：</label>
                <input type="number" id="osdop-min-duration" min="0">
                <label data-zh="最大持续时间 (ms)：" data-en="Max Duration (ms):">最大持续时间 (ms)：</label>
                <input type="number" id="osdop-max-duration" min="0">
                <button type="button" onclick="filterOSDOpTable()" data-zh="筛选" data-en="Filter">筛选</button>
                <button type="button" onclick="resetOSDOpFilter()" data-zh="重置" data-en="Reset">重置</button>
            </div>
            <div class="table-container">
            <table id="osdop-table">
                <tr>
	...
	html += `
            ...
            <div class="pagination">
                <div class="pagination-info" id="osdop-table-pagination-info"></div>
                <div class="pagination-buttons" id="osdop-table-pagination-buttons"></div>
            </div>
        </div>
    </div>`
